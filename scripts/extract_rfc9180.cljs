...
(defn- mode-section [lines n]
  (let [setup (pairs->map (parse-fields (section-lines lines (str "A.2." n ".  "))))
        encs (encryptions (parse-fields (section-lines lines (str "A.2." n ".1.  "))))
        exps (exports (parse-fields (section-lines lines (str "A.2." n ".2.  "))))]
    (assoc setup
...
(defn -main [& args]
  (let [path (first args)
    ...
    (let [lines (str/split-lines text)
          modes {:base (mode-section lines 1)
                 :psk (mode-section lines 2)
                 :auth (mode-section lines 3)
                 :auth-psk (mode-section lines 4)}
          _ (doseq [[m data] modes]
              (println (name m)
                       "fields=" (count data)
          ...
          out (str ";; GENERATED by scripts/extract_rfc9180.cljs — do not edit.\n"
                   ";;\n"
                   ";; RFC 9180 Appendix A.2 — DHKEM(X25519, HKDF-SHA256), HKDF-SHA256,\n"
                   ";; ChaCha20Poly1305 — all four modes, verbatim.\n"
                   ";;\n"
                   ";; Source:  " source-url "\n"
